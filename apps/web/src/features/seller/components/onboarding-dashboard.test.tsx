...
describe('OnboardingDashboard', () => {
  ...
  it('renders the pre-approval editor for a DRAFT seller', async () => {
    renderDashboard(
      () =>
        new Response(
          JSON.stringify({ data: { seller: onboardingStatus() }, correlationId: 'c1' }),
          {
            status: 200,
            headers: { 'Content-Type': 'application/json' },
          },
    ...
    renderDashboard(
      () =>
        new Response(
          JSON.stringify({
            data: {
              seller: onboardingStatus({ state: 'SUBMITTED', complianceState: 'IN_PROGRESS' }),
            },
            correlationId: 'c1',
          }),
  ...
  it('renders the corrections banner and resubmit action for CORRECTIONS_REQUESTED', async () => {
    ...
    expect(screen.getByText(/An administrator requested corrections/)).toBeInTheDocument();
  });
  ...
  it('creates onboarding from the start prompt', async () => {
    ...
    fireEvent.change(screen.getByLabelText(/Registration number/), {
      target: { value: 'GSTIN1234567890123' },
    });
