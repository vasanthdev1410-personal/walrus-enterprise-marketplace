...
describe('SellerApiClient', () => {
  ...
  it('sends the bearer token when a session exists', async () => {
    const fetchImpl = vi.fn().mockResolvedValue(
      jsonResponse(200, { data: { warehouses: [] }, correlationId: 'corr-1' }),
    );
  ...
  it('maps 401 to UNAUTHORIZED with a safe session-expired message', async () => {
    const fetchImpl = vi.fn().mockResolvedValue(jsonResponse(401, { success: false, message: 'AUTHENTICATION_ASSURANCE_INSUFFICIENT', errorCode: 'VALIDATION_ERROR', errors: [] }));
    const client = createClient(fetchImpl);
  ...
  it('maps 403 to ACCESS_DENIED without revealing authorization details', async () => {
    const fetchImpl = vi.fn().mockResolvedValue(jsonResponse(403, { success: false, message: 'AUTHORIZATION_DENIED', errorCode: 'UNEXPECTED_ERROR', errors: [] }));
    const client = createClient(fetchImpl);
  ...
  it('maps 404 to NOT_FOUND (non-enumerating)', async () => {
    const fetchImpl = vi.fn().mockResolvedValue(jsonResponse(404, { success: false, message: 'SELLER_NOT_FOUND', errorCode: 'RESOURCE_NOT_FOUND', errors: [] }));
    const client = createClient(fetchImpl);
    await expect(client.getOnboardingStatus()).rejects.toMatchObject({ kind: 'NOT_FOUND' });
  });

  it('maps 409 to CONFLICT (state conflict)', async () => {
    const fetchImpl = vi.fn().mockResolvedValue(jsonResponse(409, { success: false, message: 'SELLER_STATE_CONFLICT', errorCode: 'UNEXPECTED_ERROR', errors: [] }));
    const client = createClient(fetchImpl);
    await expect(client.submitOnboarding({ sellerProfileId: '0191310f-789a-7123-8123-000000000001', expectedVersion: 1 })).rejects.toMatchObject({ kind: 'CONFLICT' });
  });

  it('maps 429 to RATE_LIMITED', async () => {
    const fetchImpl = vi.fn().mockResolvedValue(jsonResponse(429, { success: false, message: 'RATE_LIMIT_EXCEEDED', errorCode: 'UNEXPECTED_ERROR', errors: [] }));
    const client = createClient(fetchImpl);
  ...
  it('maps 5xx to SERVER with a generic message', async () => {
    const fetchImpl = vi.fn().mockResolvedValue(jsonResponse(500, { success: false, message: 'An unexpected error occurred.', errorCode: 'UNEXPECTED_ERROR', errors: [] }));
    const client = createClient(fetchImpl);
  ...
  it('appends the state filter on the admin list query', async () => {
    const fetchImpl = vi.fn().mockResolvedValue(jsonResponse(200, { data: { sellers: [] }, correlationId: 'corr-1' }));
    const client = createClient(fetchImpl);
  ...
  it('builds the warehouse close path from the validated identifier', async () => {
    const fetchImpl = vi.fn().mockResolvedValue(jsonResponse(200, { data: { warehouse: { state: 'CLOSED' } }, correlationId: 'corr-1' }));
    const client = createClient(fetchImpl);
    await client.closeWarehouse({ warehouseId: '0191310f-789a-7123-8123-000000000005', expectedVersion: 5 });
    const [url] = fetchImpl.mock.calls[0] as [string, RequestInit];
  ...
  it('uses a custom base URL when configured', async () => {
    const fetchImpl = vi.fn().mockResolvedValue(jsonResponse(200, { data: { agreements: [] }, correlationId: 'corr-1' }));
  ...
  it('uses a custom idempotency-key factory for mutations', async () => {
    const fetchImpl = vi.fn().mockResolvedValue(jsonResponse(200, { data: { seller: { state: 'SUBMITTED', version: 2 } }, correlationId: 'corr-1' }));
    const client = createClient(fetchImpl, { idempotencyKeyFactory: () => 'custom-key-001' });
    await client.submitOnboarding({ sellerProfileId: '0191310f-789a-7123-8123-000000000001', expectedVersion: 1 });
    const [, init] = fetchImpl.mock.calls[0] as [string, RequestInit];
  ...
  it('reads the own profile through GET /seller/profile', async () => {
    const fetchImpl = vi.fn().mockResolvedValue(jsonResponse(200, { data: { profile: { sellerProfileId: 'id', state: 'ACTIVE' } }, correlationId: 'c1' }));
    const client = createClient(fetchImpl);
  ...
  it('updates the profile and returns the new seller state', async () => {
    const fetchImpl = vi.fn().mockResolvedValue(jsonResponse(200, { data: { seller: { state: 'DRAFT', version: 2 } }, correlationId: 'c1' }));
    const client = createClient(fetchImpl);
    await expect(
  ...
  it('reads business information', async () => {
    const fetchImpl = vi.fn().mockResolvedValue(jsonResponse(200, { data: { business: { version: 1, legalName: 'L', tradeName: 'T', businessAddress: 'A' } }, correlationId: 'c1' }));
    const client = createClient(fetchImpl);
  ...
  it('submits verification evidence references and digests only', async () => {
    const fetchImpl = vi.fn().mockResolvedValue(jsonResponse(200, { data: { verification: { state: 'SUBMITTED', generation: 1 } }, correlationId: 'c1' }));
    const client = createClient(fetchImpl);
    const result = await client.submitVerification({
      sellerProfileId: 'id',
      expectedVersion: 2,
      verificationType: 'GST',
      evidence: [{ evidenceType: 'GST_CERTIFICATE', evidenceReference: 'ref:obj', evidenceDigest: 'a'.repeat(64) }],
    });
  ...
  it('creates and closes warehouses', async () => {
    const create = vi.fn().mockResolvedValue(jsonResponse(201, { data: { warehouse: { state: 'ACTIVE' } }, correlationId: 'c1' }));
    const createClientInstance = createClient(create);
    await expect(createClientInstance.createWarehouse({ expectedVersion: 5, name: 'W', address: 'A' })).resolves.toMatchObject({ state: 'ACTIVE' });

    const close = vi.fn().mockResolvedValue(jsonResponse(200, { data: { warehouse: { state: 'CLOSED' } }, correlationId: 'c1' }));
    const closeClient = createClient(close);
    await expect(
      closeClient.closeWarehouse({ warehouseId: '0191310f-789a-7123-8123-000000000005', expectedVersion: 5 }),
    ).resolves.toMatchObject({ state: 'CLOSED' });
  });

  it('adds and removes members', async () => {
    const add = vi.fn().mockResolvedValue(jsonResponse(201, { data: { member: { associationState: 'ACTIVE' } }, correlationId: 'c1' }));
    const addClient = createClient(add);
    await expect(
      addClient.addMember({ expectedVersion: 5, memberIdentityId: '0191310f-789a-7123-8123-000000000006' }),
    ).resolves.toMatchObject({ associationState: 'ACTIVE' });

    const remove = vi.fn().mockResolvedValue(jsonResponse(200, { data: { member: { associationState: 'REMOVED' } }, correlationId: 'c1' }));
    const removeClient = createClient(remove);
    await expect(
      removeClient.removeMember({ identityId: '0191310f-789a-7123-8123-000000000006', expectedVersion: 5 }),
  ...
  it('reads the seller detail for admins', async () => {
    const fetchImpl = vi.fn().mockResolvedValue(jsonResponse(200, { data: { seller: { sellerProfileId: 'id', state: 'SUBMITTED' } }, correlationId: 'c1' }));
    const client = createClient(fetchImpl);
  ...
  it('records admin review decisions (with and without a reason)', async () => {
    const approve = vi.fn().mockResolvedValue(jsonResponse(200, { data: { seller: { state: 'APPROVED', version: 3 } }, correlationId: 'c1' }));
    ...
    const reject = vi.fn().mockResolvedValue(jsonResponse(200, { data: { seller: { state: 'REJECTED', version: 3 } }, correlationId: 'c1' }));
  ...
  it('suspends and reactivates sellers with the mandatory reason', async () => {
    const suspend = vi.fn().mockResolvedValue(jsonResponse(200, { data: { seller: { state: 'SUSPENDED', version: 6 } }, correlationId: 'c1' }));
    const suspendClient = createClient(suspend);
    await expect(
      suspendClient.suspendSeller({ sellerProfileId: 'id', expectedVersion: 5, reasonReference: 'abuse-001' }),
    ).resolves.toMatchObject({ state: 'SUSPENDED' });

    const reactivate = vi.fn().mockResolvedValue(jsonResponse(200, { data: { seller: { state: 'ACTIVE', version: 7 } }, correlationId: 'c1' }));
  ...
  it('reads evidence metadata (metadata only) for admins', async () => {
    const fetchImpl = vi.fn().mockResolvedValue(jsonResponse(200, { data: { evidence: [{ evidenceId: 'e1', evidenceType: 'GST_CERTIFICATE' }] }, correlationId: 'c1' }));
    const client = createClient(fetchImpl);
  ...
  it('reads the own verification status', async () => {
    const fetchImpl = vi.fn().mockResolvedValue(jsonResponse(200, { data: { verification: { sellerProfileId: 'id', complianceState: 'IN_PROGRESS', verifications: [] } }, correlationId: 'c1' }));
    const client = createClient(fetchImpl);
    await expect(client.getVerificationStatus()).resolves.toMatchObject({ complianceState: 'IN_PROGRESS' });
  });
});
