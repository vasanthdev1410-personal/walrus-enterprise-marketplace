...
/**
 * The order scope resolved server-side for a self-service request. The
...
export interface OrderScopedRequest extends Request {
  orderContext: OrderRequestContext;
}

/**
 * WEMP-M08-AUTHZ-001 §4 (D-08, Module 02 owner sign-off RECORDED 2026-08-20;
 * M08-M4). The order self-service permission guard:
 *
 ...
 */
@Injectable()
export class OrderSelfServicePermissionGuard implements CanActivate {
  public constructor(
  ...
  public async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    (request as OrderScopedRequest).orderContext = { customerProfileId };
    return true;
  }
}
