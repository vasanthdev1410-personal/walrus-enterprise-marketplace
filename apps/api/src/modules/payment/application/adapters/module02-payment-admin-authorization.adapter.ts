...
} from '../ports/payment-admin-authorization.port';

/**
 * WEMP-M09-AUTHZ-001 §2.2 (M09-M4). Maps every Module 09 payment
 ...
 * hidden role bypass (the engine decides; Module 09 never evaluates
 * roles itself, A-02).
 */
const ADMIN_ACTION_TO_PERMISSION: Readonly<Record<PaymentAdminAction, string>> = Object.freeze({
  'payment.admin.read': 'payment.admin.read',
  'payment.admin.manage': 'payment.admin.manage',
});

@Injectable()
export class Module02PaymentAdminAuthorizationAdapter implements PaymentAdminAuthorizationPort {
  public constructor(
    @Inject(AUTHORIZATION_APPLICATION_SERVICE)
    private readonly authorization: AuthorizationApplicationService,
  ) {}

  public async isGranted(identityId: UuidV7, action: PaymentAdminAction): Promise<boolean> {
